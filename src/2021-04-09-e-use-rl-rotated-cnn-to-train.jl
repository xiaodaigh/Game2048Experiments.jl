# the purpose of this script is to simulate the game
# and train it at the same time
# from a paper we can see that doing basic value network is sufficient to get get good performance
# since the random tile is what adds a lot of exploration
# so basic temporal difference learning is sufficient
...
# play a game from scratch based on the value of the game
@time gamerecord_nn = play_via_monte_carlo_w_recorder(group_cnn_policy; n=1, bitboard=initbboard())

s = gamerecord_nn.states[end-1]
a = gamerecord_nn.moves[end-1]

after_state=move(s, a)


sum(bitboard_to_array(after_state))
sum(bitboard_to_array(s))

normal_to_model(after_state)
